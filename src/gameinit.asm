...
;
; (1)  COLD ($A14A) is entered at initial cartridge startup (cold start).
...
;      receive keyboard input. Code execution continues into SELECT ($A15A)
;      below.
;
; (2)  SELECT ($A15A) is entered from GAMELOOP ($A1F3) after the SELECT
;      function key has been pressed. This loads the title phrase offset for the
;      copyright notice. Code execution continues into DEMO ($A15C) below.
;
; (3)  DEMO ($A15C) is entered when the game switches into demo mode. This
;      loads the demo mode flag. Code execution continues into START ($A15E)
;      below.
;
; (4)  START ($A15E) is entered from GAMELOOP ($A1F3) after the START
...
; key was pressed
;--------------------------------------
SELECT          ldy #$2F                ; Prep title phrase "COPYRIGHT ATARI 1979"


;--------------------------------------
; Entry point when game switches into
; demo mode
;--------------------------------------
DEMO            lda #$FF                ; Prep demo mode flag


;--------------------------------------
; Entry point when START function key
; was pressed
;--------------------------------------
START           sty NEWTITLEPHR         ; Enqueue new title phrase
